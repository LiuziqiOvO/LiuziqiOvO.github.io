...
RocksDB 的核心设计目标是把前台随机写改造成可批量、可顺序化、可后台整理的写入模型。它用 WAL 保障崩溃恢复，用 MemTable 吸收随机更新，用 SST 固化有序数据，用 compaction 延迟处理旧版本、删除标记和层级重叠。
...
L0 SST 来自 MemTable flush。每个 MemTable 覆盖一段时间内的写入，key range 可能与其它 MemTable 的 flush 结果重叠。直接 flush 成 L0 SST 可以让前台写入尽快释放内存，不必在 flush 阶段等待全局重分区。
...
RocksDB 的性能瓶颈经常来自 compaction 节奏与 workload 的不匹配，单次 `Put` 或 `Get` 的代码路径复杂度通常影响较小。
...
缓解方向包括明确持久化和空间回收的职责边界，避免重复 durable 路径；让上层感知下层写入特性；并对 compaction / GC / flush 做节奏控制和 I/O 限速。

## RocksDB 写路径工程点

RocksDB 的工程实现围绕三个目标展开：前台写入尽量短、读路径尽量少碰磁盘、后台 compaction 尽量不把前台延迟拖垮。多数工程优化会在 WAL、MemTable、SST、缓存和后台线程之间移动成本，LSM-Tree 模型本身通常保持不变。
